...
async def calculate_tournament_type_scores(tournament_type: TournamentType, psql_db) -> TournamentTypeResult:
    ...
    return TournamentTypeResult(
        ...
        prev_winner_won_final=prev_winner_won_final
    )
...
def tournament_scores_to_weights(tournament_scores: list[TournamentScore], prev_winner_hotkey: str | None, prev_winner_won_final: bool) -> dict[str, float]:
...
async def get_tournament_weights(psql_db) -> dict[str, float]:
    ...
    # Convert to list of TournamentScore objects
    combined_score_list = [
        TournamentScore(hotkey=hotkey, score=score)
        for hotkey, score in combined_scores.items()
    ]
    
    # Determine overall previous winner status
    # If either tournament type has a prev winner, we need to handle it
